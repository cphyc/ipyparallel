...
_metadata_keys = []
# a long time because some infinite timeouts are uninterruptible
_FOREVER = int(1e8)
        ...
        if not self._children:
            for msg_id in self.msg_ids:
                ...
                if not future:
                    ...
                    # result resides in local cache, construct already-resolved
                    # Future
                ...
                if not future:
                    raise KeyError(
                        "No Future or result for msg_id: %s" % msg_id)
    ...
    def _unordered_iter(self):
        ...
        try:
            ...
            for i in range(len(self)):
                # use very-large timeout because no-timeout is not
                # interruptible
